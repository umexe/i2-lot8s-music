...
ffmpeg -y -i "%input_video%" -i "%combined_wav%" -i "%input_image%" -map 0:v -map 1:a -c:v mpeg2video -c:a ac3 -ac 6 -b:a 448k -filter_complex "[0:v]loop=loop=-1:size=1:start=0[v];[v][2:v]overlay=0:0,scale=720:480" -t %duration% -f mpegts -pix_fmt yuv420p "%output_ts%"
